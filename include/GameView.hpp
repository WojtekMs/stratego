...
class GameView {
  ...
  bool highlight_tile(sf::RenderWindow &win, const Tile &active_unit, int to_x,
                      int to_y);
  void draw_red_init_units(sf::RenderWindow &win, bool is_dragging,
                           int selected_unit_idx, const mouse_data &m_data);
  void draw_blue_init_units(sf::RenderWindow &win, bool is_dragging,
                            int selected_unit_idx, const mouse_data &m_data);
...
public:
  ...
  const std::array<sf::Sprite, 12> &get_red_units_sprites() const {
    return red_units_sprites;
  };
  const std::array<sf::Sprite, 12> &get_blue_units_sprites() const {
    return blue_units_sprites;
  };
  ...
  void draw_units_for_init(sf::RenderWindow &win, bool is_dragging,
                           int selected_unit_idx, const mouse_data &m_data);
  void draw_sprite(sf::RenderWindow &win, int idx, int sprite_pos_x,
                   int sprite_pos_y);
