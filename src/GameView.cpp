...
GameView::GameView(Player &pA, Player &pB)
    : playerA(pA), playerB(pB), current_player(&playerA),
      other_player(&playerB), current_player_turn(Turn::PlayerA),
      board_textures_path("images/board/"),
      pieces_textures_path("images/pieces/"), TILE_SIZE(64),
      X_ADDITIONAL_SPACE(512),
      window_size(12 * TILE_SIZE + X_ADDITIONAL_SPACE, 14 * TILE_SIZE),
      sprite_initial_position(-1, -1), clock_started(false),
      done_button("Done"), randomize_button("Randomize"),
      remove_button("Remove"), end_turn_button("End Turn"),
      info_box("End of Turn"),
      attack_info_box(red_units_sprites, blue_units_sprites,
                      yellow_highlight_sprite),
...
void GameView::load_obstacle_textures() {
  for (size_t i = 0; i < obstacle_textures.size(); ++i) {
    if (i <= 3) {
      if (!obstacle_textures[i].loadFromFile(board_textures_path + "lake1_" +
                                             std::to_string(i + 1) + ".png")) {
        std::cerr << "obstacle texture lake1_" + std::to_string(i + 1) +
                         "failed to load!\n";
        abort();
      }
    } else {
      if (!obstacle_textures[i].loadFromFile(board_textures_path + "lake2_" +
                                             std::to_string(i - 3) + ".png")) {
        std::cerr << "obstacle texture lake2_" + std::to_string(i + 1) +
                         "failed to load!\n";
        abort();
      }
    }
...
void GameView::load_highlight_textures() {
  if (!red_highlight_texture.loadFromFile(board_textures_path +
                                          "highlight_red.png")) {
    std::cerr << "red highlight texture failed!\n";
    abort();
  }
  if (!yellow_highlight_texture.loadFromFile(board_textures_path +
                                             "highlight_yellow.png")) {
    std::cerr << "yellow highlight texture failed!\n";
    abort();
  }
  if (!green_highlight_texture.loadFromFile(board_textures_path +
                                            "highlight_green.png")) {
...
void GameView::load_blue_units_textures() {
  for (size_t i = 0; i < 9; ++i) {
    if (!blue_unit_textures[i].loadFromFile("images/pieces/blue/blue_" +
                                            std::to_string(i + 2) + ".png")) {
      std::cerr << "blue unit" + std::to_string(i + 2) +
  ...
  if (!blue_unit_textures[10].loadFromFile(
          "images/pieces/blue/blue_flag.png")) {
...
void GameView::load_red_units_textures() {
  for (size_t i = 0; i < 9; ++i) {
    if (!red_unit_textures[i].loadFromFile("images/pieces/red/red_" +
                                           std::to_string(i + 2) + ".png")) {
      std::cerr << "red unit" + std::to_string(i + 2) +
                       " failed to load a texture!\n";
      abort();
    }
  }
...
void GameView::draw_units_for_init(sf::RenderWindow &win, bool is_dragging,
                                   int selected_units_idx,
                                   const mouse_data &m_data) {
...
void GameView::draw_red_init_units(sf::RenderWindow &win, bool is_dragging,
  ...
  for (size_t i = 0; i < red_units_sprites.size(); ++i) {
    ...
    red_units_sprites[i].setPosition(x_denting + 2 * col_count * TILE_SIZE,
                                     y_denting + row_count * TILE_SIZE);
    text.setPosition(x_denting + TILE_SIZE + 2 * col_count * TILE_SIZE,
                     y_denting + TILE_SIZE / 4 + row_count * TILE_SIZE);
    text.setString("x" +
                   std::to_string(playerA.get_board().get_max_unit_count(i) -
                                  playerA.get_units_count(i)));
    if (is_dragging == true) {
      red_units_sprites[selected_unit_idx].setPosition(
          m_data.mouse_x - m_data.mouse_object_offset_x,
          m_data.mouse_y - m_data.mouse_object_offset_y);
    }
...
void GameView::draw_blue_init_units(sf::RenderWindow &win, bool is_dragging,
                                    int selected_unit_idx,
                                    const mouse_data &m_data) {
  const int x_denting = board_border.getGlobalBounds().width;
  const int y_denting = TILE_SIZE;
  ...
  for (size_t i = 0; i < blue_units_sprites.size(); ++i) {
    ...
    blue_units_sprites[i].setPosition(x_denting + 2 * col_count * TILE_SIZE,
                                      y_denting + row_count * TILE_SIZE);
    text.setPosition(x_denting + TILE_SIZE + 2 * col_count * TILE_SIZE,
                     y_denting + TILE_SIZE / 4 + row_count * TILE_SIZE);
    text.setString("x" +
                   std::to_string(playerB.get_board().get_max_unit_count(i) -
                                  playerB.get_units_count(i)));
    if (is_dragging == true) {
      blue_units_sprites[selected_unit_idx].setPosition(
          m_data.mouse_x - m_data.mouse_object_offset_x,
          m_data.mouse_y - m_data.mouse_object_offset_y);
    }
...
void GameView::draw_done_button(sf::RenderWindow &win) {
  const int x_denting = board_border.getGlobalBounds().width;
  done_button.set_position(
      x_denting + (X_ADDITIONAL_SPACE - done_button.get_width()) / 2,
      TILE_SIZE * 6);
  done_button.draw(win);
}
void GameView::draw_randomize_button(sf::RenderWindow &win) {
  randomize_button.set_position(done_button.get_position().x,
                                done_button.get_position().y +
                                    done_button.get_height() + TILE_SIZE);
  randomize_button.draw(win);
}

void GameView::draw_remove_button(sf::RenderWindow &win) {
  remove_button.set_position(randomize_button.get_position().x,
                             randomize_button.get_position().y +
                                 randomize_button.get_height() + TILE_SIZE);
...
void GameView::draw_info_box(sf::RenderWindow &win) {
  ...
  info_box.set_position((win.getSize().x - info_box.get_width()) / 2,
                        (win.getSize().y - info_box.get_height()) / 2);
  info_box.draw(win);
}

void GameView::draw_attack_info_box(sf::RenderWindow &win,
                                    const std::optional<Unit> &attacker,
                                    const std::optional<Unit> &attacked) {
  if (!clock_started) {
    clock.restart();
    clock_started = true;
  }
  sf::Time time_past = clock.getElapsedTime();
  attack_info_box.set_position(
      (window_size.x - attack_info_box.get_width()) / 2,
      (window_size.y - attack_info_box.get_height()) / 2);
...
void GameView::draw_unit_highlight(sf::RenderWindow &win,
                                   const Tile &active_unit) {
  yellow_highlight_sprite.setPosition(
      return_pixels(active_unit.x, active_unit.y));
  win.draw(yellow_highlight_sprite);
}

void GameView::draw_sprite(sf::RenderWindow &win, int idx, int sprite_pos_x,
                           int sprite_pos_y) {
  if (idx == 12) {
    if (current_player_turn == Turn::PlayerA) {
...
void GameView::draw_possible_moves_for_active_unit(sf::RenderWindow &win,
                                                   const Tile &active_unit) {
  const auto &unit =
      current_player->get_board().get_unit(active_unit.x, active_unit.y);
  if (!unit) {
    return;
  }
  std::visit(overloads{
                 [this, &win, &active_unit](const ScoutUnit &) {
                   highlight_scout_moves(win, active_unit);
                 },
                 [this, &win, &active_unit](const auto &) {
...
void GameView::highlight_scout_moves(sf::RenderWindow &win,
                                     const Tile &active_unit) {
  for (int col = active_unit.x + 1;
       col < current_player->get_board().get_width(); ++col) {
  ...
  for (int row = active_unit.y + 1;
       row < current_player->get_board().get_height(); ++row) {
...
bool GameView::highlight_tile(sf::RenderWindow &win, const Tile &active_unit,
                              int to_x, int to_y) {
...
void GameView::draw_end_game_screen(sf::RenderWindow &win,
                                    const std::string &victorious_name) {
  end_game_info_box.set_text("Congratulations " + victorious_name +
                             " you won!\n");
