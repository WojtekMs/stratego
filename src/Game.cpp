...
Game::Game(GameView &g_view, GameController &g_controller)
    : game_view(g_view), game_controller(g_controller), clock_started(false) {}

std::string Game::run(sf::RenderWindow &win) {
  while (win.isOpen()) {
    sf::Event event;
    game_view.update_players(game_controller.get_current_player(),
                             game_controller.get_other_player());
...
void Game::draw(sf::RenderWindow &win) {
  if (game_controller.get_game_finished()) {
    game_view.draw_end_game_screen(
        win, game_controller.get_victorious_player_name());
    return;
  }
  ...
  if (game_controller.get_unit_attacked()) {
    game_view.draw_attack_info_box(win, game_controller.get_attacker(),
                                   game_controller.get_attacked());
    return;
  }
  ...
  if (!game_controller.both_boards_set()) {
    game_view.draw_units_for_init(win, game_controller.get_dragging(),
                                  game_controller.get_selected_init_unit_idx(),
    ...
    if (game_controller.get_is_active_unit() &&
        !game_controller.get_unit_moved_this_round()) {
      game_view.draw_possible_moves_for_active_unit(
          win, game_controller.get_active_unit());
    }
  }
}
