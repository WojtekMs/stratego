...
void Board::unset_unit(Tile tile) { units[tile.y][tile.x] = std::nullopt; }
...
void Board::set_state(State state) { current_state = state; }
...
Tile Board::point_reflection(int col, int row) {
  ...
  return Tile(reflection_point.x + reflection.x,
              reflection_point.y + reflection.y);
}
