...
MainMenu::MainMenu(Player &pA, Player &pB)
    : playerA(pA), playerB(pB), start_game_button("Start Game"),
      game_rules_button("Game Rules"), exit_button("Exit The Game"),
      next("Next"), prev("Previous"), names_entered(false),
      game_started_button_pressed(false), rules_button_pressed(false),
      exit_button_pressed(false), next_button_pressed(false),
      prev_button_pressed(false), player_1_name_approved(false),
      player_2_name_approved(false), entering_from_menu(true),
      path_to_textures("images/board/"), idx(0) {
  for (size_t i = 0; i < rules_textures.size(); ++i) {
    if (!rules_textures[i].loadFromFile(path_to_textures + "rules#" +
                                        std::to_string(i + 1) + ".png")) {
      std::cerr << "rules#" + std::to_string(i + 1) +
...
void MainMenu::set_buttons_pressed() {
  if (start_game_button.is_highlighted() && !rules_button_pressed &&
      !exit_button_pressed) {
  ...
  if (game_rules_button.is_highlighted() && !game_started_button_pressed &&
      !exit_button_pressed) {
  ...
  if (exit_button.is_highlighted() && !game_started_button_pressed &&
...
void MainMenu::draw_menu(sf::RenderWindow &win) {
  logo_sprite.setPosition(
      (win.getSize().x - logo_sprite.getLocalBounds().width) / 2,
      0.8 * start_game_button.get_height());
  start_game_button.set_position(
      (win.getSize().x - start_game_button.get_width()) / 2,
  ...
  game_rules_button.set_position(start_game_button.get_position().x,
                                 start_game_button.get_position().y +
                                     1.25 * game_rules_button.get_height());
  exit_button.set_position(game_rules_button.get_position().x,
                           game_rules_button.get_position().y +
                               1.25 * exit_button.get_height());
...
void MainMenu::draw_names(sf::RenderWindow &win) {
  if (!player_1_name_approved) {
    ...
    player_1_name.set_position(
        (win.getSize().x - player_1_name.get_width()) / 2,
...
void MainMenu::draw_rules(sf::RenderWindow &win) {
  ...
  next.set_position(win.getSize().x - 1.25 * next.get_width(),
                    win.getSize().y - 1.75 * next.get_height());
  prev.set_position(0.25 * next.get_width(),
                    win.getSize().y - 1.75 * next.get_height());
  draw_rule(win, idx);
...
void MainMenu::draw_rule(sf::RenderWindow &win, int idx) {
  ...
  int rules_x_pos =
      (win.getSize().x - rules_sprites[idx].getLocalBounds().width) / 2;
  int rules_y_pos =
      (win.getSize().y - rules_sprites[idx].getLocalBounds().height) / 2;
...
void MainMenu::highlight_buttons(int mouse_x, int mouse_y) {
  ...
  if (player_1_name.button_contains(mouse_x, mouse_y) &&
      game_started_button_pressed) {
    player_1_name.set_button_highlight_on();
  } else {
    player_1_name.set_button_highlight_off();
  }
  if (player_2_name.button_contains(mouse_x, mouse_y) &&
      player_1_name_approved) {
